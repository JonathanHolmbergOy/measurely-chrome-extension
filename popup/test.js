    ...
    const METRICS = {
        Performance: {
            FCP: {
                ...
                calculate: (data) => {
                    if (!data.fcp || data.fcp.length === 0) return { value: null, issues: [] };
                
                    const fcpTime = data.fcp[0].time;
                    const issues = [{
                        type: 'fcp-info',
                        time: fcpTime.toFixed(0),
                        info: `First Contentful Paint occurred at ${fcpTime.toFixed(0)}ms`
                    }];
                
                    return { value: fcpTime, issues };
                }
            },
            ...
            LCP: {
                ...
                calculate: (data) => {
                    if (!data.lcp || data.lcp.length === 0) return { value: null, issues: [] };
                
                    const finalLCP = data.lcp[data.lcp.length - 1];
                    const issues = [];
                
                    if (finalLCP.element) {
                        let info = `LCP element: ${finalLCP.element}${finalLCP.id ? `#${finalLCP.id}` : ''} (${finalLCP.size}px²)`;
                        let fix = null;
                    
                        if ((finalLCP.element === 'P' || finalLCP.element === 'H1' || finalLCP.element === 'H2' || finalLCP.element === 'DIV') && finalLCP.size > 5000) {
                    ...
                    return { value: finalLCP.time, issues };
                }
            },
                ...
                setup: (data) => {
                    try {
                        ...
                        if (navEntries.length > 0) {
                            ...
                            if (ttfb > 0) data.ttfb.push(breakdown);
                        }
                    } catch (e) {
                        console.warn('TTFB setup failed:', e);
                    }
                },
                calculate: (data) => {
                    if (!data.ttfb || data.ttfb.length === 0) return { value: null, issues: [] };
                
                    const breakdown = data.ttfb[0];
                    const issues = [{
                        type: 'ttfb-breakdown',
                        ttfb: breakdown.ttfb.toFixed(0),
                        redirect: breakdown.redirect.toFixed(0),
                        dns: breakdown.dns.toFixed(0),
                        tcp: breakdown.tcp.toFixed(0),
                        ssl: breakdown.ssl.toFixed(0),
                        request: breakdown.request.toFixed(0),
                        info: `TTFB breakdown: DNS ${breakdown.dns.toFixed(0)}ms + TCP ${breakdown.tcp.toFixed(0)}ms + SSL ${breakdown.ssl.toFixed(0)}ms + Request ${breakdown.request.toFixed(0)}ms`
                    }];
                
                    return { value: breakdown.ttfb, issues };
                }
            },

            TTI: {
                title: 'Time to Interactive',
                description: 'Time until page is fully interactive (approximation using domInteractive)',
                ...
                setup: (data) => {
                    try {
                        ...
                        if (navEntries.length > 0) {
                            const nav = navEntries[0];
                            const tti = nav.domInteractive || nav.domContentLoadedEventEnd;
                            if (tti > 0) data.tti.push(tti);
                        }
                    } catch (e) {
                        console.warn('TTI setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const stats = calculateStats(data.tti);
                    const issues = [{
                        type: 'info',
                        info: 'TTI approximation using domInteractive. True TTI requires 5s quiet window (no long tasks > 50ms)'
                    }];
            ...
            TBT: {
                ...
                setup: (data) => {
                    try {
                        ...
                        console.warn('TBT observer setup failed:', e);
                    }
                },
                calculate: (data) => {
                    ...
                    const longTasks = data.tbt
                        .sort((a, b) => b.blocking - a.blocking)
                        .slice(0, 5)
                        .map(task => ({
                            type: 'long-task',
                            duration: task.duration.toFixed(0),
                            blocking: task.blocking.toFixed(0),
                            name: task.name || 'unknown',
                            info: `Long task: ${task.duration.toFixed(0)}ms (${task.blocking.toFixed(0)}ms blocking)`
                        }));
                
                    return { value: totalBlocking, issues: longTasks };
                }
            },

            FMP: {
                title: 'First Meaningful Paint',
                description: 'Time when primary content becomes visible',
                ...
                setup: (data) => {
                    try {
                        ...
                        if (navEntries.length > 0) {
                            const nav = navEntries[0];
                            const fmp = nav.domContentLoadedEventEnd || nav.responseEnd;
                            if (fmp > 0) data.fmp.push(fmp);
                        }
                    } catch (e) {
                        console.warn('FMP setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const stats = calculateStats(data.fmp);
                    return { value: stats.avg || null, issues: [] };
                }
            },
                ...
                setup: (data) => {
                    try {
                        ...
                        resources.forEach(resource => {
                            if (resource.duration > 0 && (resource.initiatorType === 'link' || resource.initiatorType === 'img')) {
                                data.resourceLoad.push(resource.duration);
                            }
                        });
                    } catch (e) {
                        console.warn('Resource load setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const stats = calculateStats(data.resourceLoad);
                    return { value: stats.avg || null, issues: [] };
                }
            },
            ...
            JSE: {
                ...
                description: 'Total time spent executing JavaScript',
                    ...
                    try {
                        ...
                        resources.forEach(resource => {
                            if (resource.initiatorType === 'script' && resource.duration > 0) {
                                data.jsExecution.push(resource.duration);
                            }
                        });
                    } catch (e) {
                        console.warn('JS execution setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const totalTime = data.jsExecution.length > 0 ? data.jsExecution.reduce((sum, val) => sum + val, 0) : 0;
                    ...
                    try {
                        ...
                        resources.forEach(resource => {
                            if ((resource.initiatorType === 'xmlhttprequest' || resource.initiatorType === 'fetch') && resource.duration > 0) {
                                data.networkTime.push(resource.duration);
                            }
                        });
                    } catch (e) {
                        console.warn('Network time setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const stats = calculateStats(data.networkTime);
                    return { value: stats.avg || null, issues: [] };
                }
            },
            ...
            FID: {
                ...
                calculate: (data) => {
                    const stats = calculateStats(data.fid);
                    return { value: stats.avg || null, issues: [] };
                }
            },
                    ...
                    try {
                        ...
                        setTimeout(() => {
                            try {
                                ...
                                element.id = 'webdoctor-inp-trigger';
                        ...
                        console.warn('INP observer setup failed:', e);
                    }
                },
                calculate: (data) => {
                    ...
                    try {
                        ...
                        resources.forEach(resource => {
                            if (resource.domainLookupStart !== undefined && resource.domainLookupEnd !== undefined) {
                                ...
                                if (dns >= 0) data.dnsLookup.push(dns);
                            }
                        });
                    } catch (e) {
                        console.warn('DNS lookup setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const stats = calculateStats(data.dnsLookup);
                    ...
                    try {
                        ...
                        resources.forEach(resource => {
                            if (resource.connectStart !== undefined && resource.connectEnd !== undefined) {
                                ...
                                if (tcp >= 0) data.tcpTime.push(tcp);
                            }
                        });
                    } catch (e) {
                        console.warn('TCP time setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const stats = calculateStats(data.tcpTime);
                    return { value: stats.avg !== null ? stats.avg : null, issues: [] };
                }
            },
                    ...
                    try {
                        ...
                        resources.forEach(resource => {
                            if (resource.secureConnectionStart !== undefined && resource.secureConnectionStart > 0 && resource.connectEnd !== undefined) {
                                ...
                                if (ssl >= 0) data.sslTime.push(ssl);
                            }
                        });
                    } catch (e) {
                        console.warn('SSL time setup failed:', e);
                    }
                },
                calculate: (data) => {
                    const stats = calculateStats(data.sslTime);
                    return { value: stats.avg !== null ? stats.avg : null, issues: [] };
                }
            },
            ...
            ST: {
                        ...
                        serverEntries.forEach(entry => {
                            if (entry.serverTiming && entry.serverTiming.length > 0) {
                                entry.serverTiming.forEach(st => {
                                    if (st.duration !== undefined) data.serverTiming.push(st.duration);
                                });
                            }
                        });
                    } catch (e) {}
                ...
                calculate: (data) => {
                    const stats = calculateStats(data.serverTiming);
                    return { value: stats.avg || null, issues: [] };
                }
            },
            ...
            ET: {
                ...
                setup: (data) => {
                    try {
                        ...
                        entries.forEach(e => { if (e.renderTime) data.elementTiming.push(e.renderTime); });
                    } catch (e) {}
                },
                calculate: (data) => {
                    const stats = calculateStats(data.elementTiming);
                    return { value: stats.avg || null, issues: [] };
                }
            },
            ...
            RTT: {
                ...
                description: 'DNS, TCP, and SSL timing breakdown',
                threshold: { good: 100, poor: 300 },
            ...
            RHP: {
                ...
                setup: (data) => {
                    try {
                        ...
                        console.warn('Resource hints setup failed:', e);
                    }
                },
                calculate: (data) => {
            ...
            TPI: {
                ...
                setup: (data) => {
                    try {
                        ...
                        data.thirdPartyScore = time + (size / 1000);
                    } catch (e) {}
                },
                calculate: (data) => {
                    return { value: data.thirdPartyScore, issues: [] };
                }
            },
            ...
            CRU: {
                ...
                setup: (data) => {
                    try {
                        ...
                        data.connReuse = total > 0 ? (reused / total) * 100 : 100;
                    } catch (e) {}
                },
                calculate: (data) => {
                    return { value: data.connReuse, issues: [] };
                }
            },
            ...
            CSO: {
                ...
                calculate: (data) => {
                    return {
                        ...
                        issues: data.largeBundles.map(b => ({ type: 'large-bundle', resource: b.name, size: b.transferSize, fix: 'Split bundle' }))
                    };
                }
            },
            ...
            LIL: {
                ...
                setup: (data) => {
                    try {
                        ...
                        data.lazyPercent = belowFold > 0 ? (lazy / belowFold) * 100 : 100;
                    } catch (e) {}
                },
                calculate: (data) => {
                    return { value: data.lazyPercent, issues: [] };
                }
            },
            ...
            RBR: {
                ...
                setup: (data) => {
                    try {
                        ...
                        console.warn('Render-blocking setup failed:', e);
                    }
                },
                calculate: (data) => {
            ...
            XCT: {
                ...
                setup: async (data) => {
                    try {
                        ...
                        console.warn('XCT header fetch failed:', e);
